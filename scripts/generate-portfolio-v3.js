...
function copyTable(tableId){var t=document.getElementById(tableId);var rows=t.querySelectorAll('tr');var tsv=[];rows.forEach(function(r){var cells=r.querySelectorAll('th,td');var row=[];cells.forEach(function(c){row.push(c.textContent.trim());});tsv.push(row.join('\\t'));});navigator.clipboard.writeText(tsv.join('\\n')).then(function(){var msgId=tableId==='tbl-incons'?'copy-msg':'copy-'+tableId.replace('tbl-','')+'-msg';var m=document.getElementById(msgId);if(m){m.style.display='inline';setTimeout(function(){m.style.display='none';},2000);}});}
function downloadTableCSV(){var t=document.getElementById('tabla-iniciativas');var rows=t.querySelectorAll('tr');var csv='\\uFEFF';rows.forEach(function(r){var cells=r.querySelectorAll('th,td');var row=[];cells.forEach(function(c){var v=c.textContent.trim().replace(/"/g,'""');row.push('"'+v+'"');});csv+=row.join(',')+'\\n';});var blob=new Blob([csv],{type:'text/csv;charset=utf-8;'});var url=URL.createObjectURL(blob);var a=document.createElement('a');a.href=url;a.download='portafolio-iniciativas-'+new Date().toISOString().slice(0,10)+'.csv';a.click();URL.revokeObjectURL(url);}</script></body></html>`;
  ...
  if (useDynamic) {
    ...
    for (const ini of INI) {
      ...
      for (const ik of iniKeys) {
        try {
          const ikUrl = `${JIRA_BASE}/rest/api/3/issue/${ik}?fields=summary,customfield_25475,customfield_25476,customfield_25632,duedate`;
          ...
          if (iniKeys.length === 1 && ikResp.fields.summary) {
            ini[2] = ikResp.fields.summary.trim();
          }
